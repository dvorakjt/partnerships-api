...
CREATE TABLE reward (
	...
	CONSTRAINT reward_uses_a_single_availability_mode CHECK (
		(
			available_from_exact IS NULL
			AND available_until_exact IS NULL
		)
		OR (
			available_from_local IS NULL
			AND available_until_local IS NULL
		)
	),
	CONSTRAINT reward_exact_availability_is_well_ordered CHECK (
		available_from_exact IS NULL
		OR available_until_exact IS NULL
		OR available_from_exact <= available_until_exact
	),
	CONSTRAINT reward_local_availability_is_well_ordered CHECK (
		available_from_local IS NULL
		OR available_until_local IS NULL
		OR available_from_local <= available_until_local
	)
...
/*
	This table contains only rewards whose partner is active and that have 
	translated details in all supported languages.
*/
...
BEGIN
	RETURN COALESCE((
		SELECT voucher_type = reward_voucher_type_matches.expected_voucher_type
		FROM reward
...
DROP TRIGGER reward_details_translation_update_trigger ON reward_details_translation;
DROP TRIGGER reward_category_update_trigger ON reward_category;
DROP TRIGGER reward_update_trigger ON reward;
...
DROP FUNCTION refresh_valid_reward_table_for_category(INT);
DROP FUNCTION refresh_valid_reward_table_for_partner(INT);
DROP FUNCTION refresh_valid_reward_table_for_reward(UUID);
DROP FUNCTION is_valid_reward(UUID);
DROP FUNCTION reward_voucher_type_matches(UUID, voucher_type);
